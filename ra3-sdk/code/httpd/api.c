	...
	char	*str;
{
        ...
        new = (char *)httpd_escape_string(str);
	if (new == NULL)
	{
		return(NULL);
	}
	...
	httpContent *entry;
{
	if (entry) {
		...
		if (entry->next)
			freeContent(entry->next);

		entry->next = NULL;
		entry->data = entry->name = entry->path = NULL;
	...
	httpDir	*dir;
{
	if (dir) {
		if (dir->children)
			freeDir(dir->children);

		if (dir->entries)
			freeContent(dir->entries);
	...
	httpd	*server;
{
	...
	{
		shutdown(server->serverSock, 2);
		close(server->serverSock);
	}

	if (server->clientSock)
	{
	...
	struct	timeval *timeout;
{
	...
	no_timeout.tv_usec = no_timeout.tv_sec = 0;
	...
	httpd	*server;
{
	...
	*/
	strcpy(server->response.headers, "Server: RA3 Embedded HTTPd\n");
	...
	*/
	count = 0;
	inHeaders = 1;
	while(httpd_readLine(server, buf, HTTP_MAX_LEN) > 0)
	{
		count++;
		...
		if (count == 1)
		{
			...
			*cp2 = 0;
			...
			if (server->request.method == 0)
			{
				...
				httpd_net_write( server->clientSock, cp,
				      strlen(cp));
				httpd_writeErrorLog(server,LEVEL_ERROR,
			...
			*cp2 = 0;
			...
			httpd_cleanUrl(server->request.path);
			continue;
		}
	...
	*/
	if (server->request.contentLength > 0)
	{
		...
		httpd_readBuf(server, buf, server->request.contentLength);
		httpd_storeData(server, buf);
...
void httpdEndRequest(server)
	httpd	*server;
{
	httpd_freeVariables(server->variables);
	...
	char	*path;
{
	...
	dirPtr = httpd_findContentDir(server, dir, HTTP_TRUE);
	...
	char	*msg;
{
	...
	*dest = 0;
	...
	httpd_net_write( server->clientSock, buf, strlen(buf));
}
	...
	httpd	*server;
{
	...
	if (cp[1])
		...
		strncpy(entryName, "index.html", HTTP_MAX_URL);
	if (cp != dirName)
		*cp = 0;
	else
		*(cp+1) = 0;
	dir = httpd_findContentDir(server, dirName, HTTP_FALSE);
	if (dir == NULL)
	{
		httpd_send404(server);
		httpd_writeAccessLog(server);
		return;
	}
	entry = httpd_findContentEntry(server, dir, entryName);
	if (entry == NULL)
	{
		httpd_send404(server);
		httpd_writeAccessLog(server);
	...
	switch(entry->type)
	{
		...
		case HTTP_STATIC:
			httpd_sendStatic(server, entry->data);
			break;

		case HTTP_FILE:
			httpd_sendFile(server, entry->path);
			break;

		case HTTP_WILDCARD:
			if (httpd_sendDirectoryEntry(server,entry,entryName)<0)
			{
				httpd_send404(server);
			}
			break;
